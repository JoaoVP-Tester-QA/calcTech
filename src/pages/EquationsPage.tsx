...
import { bolzanoMethod, bisectionMethod, newtonRaphsonMethod, secantMethod } from '../utils/calculations';
import 'mathlive';
...
const EquationsPage: React.FC = () => {
  ...
  const getMethodName = () => {
    switch (method) {
      case 'Bolzano': return 'Bolzano';
      case 'Bissecção': return 'Bissecção';
      case 'Newton-Raphson': return 'Newton-Raphson';
      case 'Secante': return 'Secante';
    ...
    setTimeout(() => {
      ...
      try {
        if (method === 'bolzano') {
          ...
          calculatedResult = bisectionMethod(functionInput, intervalA, intervalB);
        } else if (method === 'newton') {
          calculatedResult = newtonRaphsonMethod(functionInput, "derivadaDaFuncaoAqui", initialGuess);
        } else if (method === 'secant') {
          calculatedResult = secantMethod(functionInput, intervalA, intervalB);
        }
      } catch (error) {
        calculatedResult = "Erro ao processar a função. Verifique a entrada e tente novamente.";
      }
  ...
  return (
    <div>
      <h2>Resolução de Equações Lineares</h2>
      <h3>Método escolhido: {getMethodName()}</h3>

      {/* <math-field
        ref={mathFieldRef}
        style={{ width: '100%', padding: '8px', fontSize: '18px' }}
...
<input
        ...
        placeholder="Insira a função aqui"
      />
<MathKeyboard onInput={(input) => setFunctionInput((prev) => prev + input)} /> 
      ...
      {(method === 'bolzano' || method === 'bisection' || method === 'secant') && (
        <>
          <input
            type="number"
            value={intervalA}
